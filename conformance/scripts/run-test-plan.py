    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            if state == "WAITING":
                ...
                elif re.match(r'oid4vci-1_0-issuer-.*', module):
                    # Repo-local patch (not upstream) -- see conformance/README.md "Patches on top of
                    # the vendored scripts". oid4vci-demo-issuer's own /authorize endpoint has no real
                    # login/consent screen (DemoAuthorizationClaimsResolver auto-approves; confirmed
                    # live: a plain, unauthenticated GET that follows redirects completes in one 302
                    # straight to the suite's own callback URL). That callback response is itself an
                    # "implicitCallback" HTML page (templates/implicitCallback.html) whose JS reads
                    # window.location.hash and XHR-POSTs it (Content-Type: text/plain) to a separate
                    # implicit_submit.fullUrl -- served unconditionally by the suite for every callback,
                    # not just fragment-based (implicit/hybrid) flows. Our flow is response_type=code
                    # with plain query-string params (no fragment; confirmed live the query params were
                    # already recorded server-side before this page was even served), so the equivalent
                    # of "no hash to send" is POSTing an empty body there. Repeats for as long as the
                    # module keeps re-entering WAITING (e.g. oid4vci-1_0-issuer-happy-flow-additional-requests
                    # does more than one authorization round), bounded so a genuinely stuck module still
                    # gives up rather than looping forever.
                    #
                    # Race condition (confirmed live): the module's status can flip to WAITING slightly
                    # before the "Redirecting to authorization endpoint" entry is actually appended to its
                    # log, since that log write and the status-change notification this outer loop reacts
                    # to aren't atomic. A single immediate get_test_log() can therefore see neither a
                    # redirect nor an implicit_submit yet and wrongly conclude there's nothing to do. Retry
                    # a few times with a short delay before actually giving up, rather than bailing out on
                    # the first empty check.
                    print('[oid4vci-patch] {} entered WAITING -- {}log-detail.html?log={}'.format(module_id, api_url_base, module_id))
                    already_followed_ids = set()
                    empty_checks = 0
                    for check_num in range(1, 11):
                        async with httpx.AsyncClient(follow_redirects=True, timeout=20) as browser:
                            module_log = await conformance.get_test_log(module_id)
                            redirect_entry = next(
                                (e for e in reversed(module_log)
                                 if e.get('http') == 'redirect' and 'redirect_to' in e and e.get('_id') not in already_followed_ids),
                                None)
                            if redirect_entry is not None:
                                already_followed_ids.add(redirect_entry.get('_id'))
                                redirect_to = redirect_entry['redirect_to']
                                print('[oid4vci-patch] check {}: following authorization redirect (no real browser needed for this issuer): {}'.format(check_num, redirect_to))
                                browser_resp = await browser.get(redirect_to)
                                print('[oid4vci-patch] redirect chain finished with HTTP {} at {}'.format(browser_resp.status_code, browser_resp.url))
                                module_log = await conformance.get_test_log(module_id)

                            implicit_entry = next(
                                (e for e in reversed(module_log)
                                 if 'implicit_submit' in e and e.get('_id') not in already_followed_ids),
                                None)
                            if implicit_entry is not None:
                                already_followed_ids.add(implicit_entry.get('_id'))
                                implicit_submit_url = implicit_entry['implicit_submit']['fullUrl']
                                print('[oid4vci-patch] check {}: submitting empty implicit-callback body (no fragment for a query-based code response): {}'.format(check_num, implicit_submit_url))
                                await browser.post(implicit_submit_url, content=b'', headers={'Content-Type': 'text/plain'})

                            if redirect_entry is None and implicit_entry is None:
                                empty_checks += 1
                                last_entry = module_log[-1] if module_log else None
                                last_desc = '{} / {}'.format(last_entry.get('src'), last_entry.get('msg')) if last_entry else '(empty log)'
                                print('[oid4vci-patch] check {}: no new redirect/implicit entry yet ({} log entries, last: {}) -- {}'.format(
                                    check_num, len(module_log), last_desc,
                                    'giving up after this, falling back to plain wait_for_state' if empty_checks >= 5 else 'retrying in 1s'))
                                if empty_checks >= 5:
                                    break
                            ...
                            empty_checks = 0
                        state = await conformance.wait_for_state(module_id, ["WAITING", "FINISHED"])
                        print('[oid4vci-patch] check {}: module status after this round is {}'.format(check_num, state))
                        if state == "FINISHED":
                            break
